...
export const postScreeningMessage = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const reserveResult = await platformPool.query(
    ...
    if (reserveResult.rows.length === 0) {
      ...
      const diagResult = await platformPool.query(`SELECT * FROM screening_sessions WHERE id = $1`, [id]);
    ...
    if (shouldComplete) {
      const moved = await applyAiStageMove(db, {
        tenantId: session.tenant_id,
