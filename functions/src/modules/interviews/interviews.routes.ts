...
const readRoles  = ["super_admin", "accounts_manager", "recruiter", "vendor_manager"];

router.post("/", authMiddleware, tenantIsolation, authorize(writeRoles), interviewController.createInterview);
router.get("/applications/:applicationId", authMiddleware, tenantIsolation, authorize(readRoles), interviewController.getInterviewsByApplication);
