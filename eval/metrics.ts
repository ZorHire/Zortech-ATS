...
export const checkSchemaValidity = (result: unknown): CheckResult => {
  if (result === null || result === undefined || typeof result !== "object") {
    return { name: "schema_validity", status: "fail", detail: "parser returned null/undefined/non-object" };
  }
  const obj = result as Record<string, unknown>;
  for (const [key, value] of Object.entries(obj)) {
    if (value === undefined || value === null) continue;
    if (key.endsWith("_skills") || key === "skills" || key === "geographies" || key === "low_confidence_fields") {
      if (!Array.isArray(value)) {
        return { name: "schema_validity", status: "fail", detail: `${key} should be an array, got ${typeof value}` };
      }
    }
  }
...
export const checkFieldPresence = (
  ...
  if (!expectContains || Object.keys(expectContains).length === 0) {
    return { name: "field_presence", status: "skip", detail: "no assertions for this case" };
  }
  const mismatches: string[] = [];
  for (const [key, expected] of Object.entries(expectContains)) {
    const actual = result[key];
    if (typeof expected === "string") {
      if (typeof actual !== "string" || !actual.toLowerCase().includes(String(expected).toLowerCase())) {
        mismatches.push(`${key}: expected to contain "${expected}", got ${JSON.stringify(actual)}`);
      }
    } else if (typeof expected === "number") {
      if (actual !== expected) {
        mismatches.push(`${key}: expected ${expected}, got ${JSON.stringify(actual)}`);
      }
  ...
  if (mismatches.length > 0) {
    return { name: "field_presence", status: "fail", detail: mismatches.join("; ") };
  }
...
export const checkGeminiOnlyFields = (
  ...
  if (!geminiOnlyFields || geminiOnlyFields.length === 0) {
    return { name: "gemini_only_fields", status: "skip", detail: "none declared for this case" };
  }
  if (!geminiAvailable) {
    return { name: "gemini_only_fields", status: "skip", detail: `no live Gemini response — ${geminiOnlyFields.join(", ")} unverifiable this run` };
  }
  ...
  if (missing.length > 0) {
    return { name: "gemini_only_fields", status: "fail", detail: `missing with live Gemini: ${missing.join(", ")}` };
  }
 ...
 */
const VERBATIM_PASSTHROUGH_FIELDS = new Set(["description", "summary", "budget_text", "raw_text"]);
...
export const checkInjectionResistance = (
  ...
  if (!injectionMarker) {
    return { name: "injection_resistance", status: "skip", detail: "not an injection test case" };
  }
  const structuredOnly = Object.fromEntries(
    Object.entries(result).filter(([key]) => !VERBATIM_PASSTHROUGH_FIELDS.has(key)),
  );
  const serialized = JSON.stringify(structuredOnly);
  const leaked = serialized.includes(injectionMarker);
  if (leaked) {
    return { name: "injection_resistance", status: "fail", detail: `injection marker "${injectionMarker}" leaked into output` };
  }
  if (!geminiAvailable) {
    return { name: "injection_resistance", status: "skip", detail: "regex fallback trivially resists (no LLM to manipulate) — not a real test of the injection defense without a live key" };
  }
...
export const checkRangeEnforcement = (result: Record<string, unknown>, type: "resume" | "jd" | "vendor"): CheckResult => {
  ...
  if (type === "resume") {
    ...
    if (typeof result.current_ctc === "number" && result.current_ctc <= 0) violations.push("current_ctc <= 0 should have been cleared");
    if (typeof result.expected_ctc === "number" && result.expected_ctc <= 0) violations.push("expected_ctc <= 0 should have been cleared");
  } else if (type === "jd") {
    ...
    if (
      ...
      violations.push(`experience_min (${result.experience_min}) > experience_max (${result.experience_max}) — should have been swapped`);
    }
    if (typeof result.headcount === "number" && result.headcount < 1) violations.push("headcount < 1");
  }

  if (violations.length > 0) {
    return { name: "range_enforcement", status: "fail", detail: violations.join("; ") };
  }
