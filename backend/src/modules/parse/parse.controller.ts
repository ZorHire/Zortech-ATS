...
export const parseResume = async (req: AuthRequest, res: Response) => {
  ...
  console.log("Resume file:", req.file?.originalname, req.file?.mimetype);
  try {
    const file = req.file;
    if (!file) {
    ...
    if (!file.buffer || !Buffer.isBuffer(file.buffer)) {
      console.warn("Resume parse warning: file.buffer missing or invalid", {
    ...
    // Surface actionable errors (e.g. unsupported .doc format) to the frontend
...
export const parseJobDescription = async (req: AuthRequest, res: Response) => {
  ...
  console.log("JD file:", req.file?.originalname, req.file?.mimetype);
  try {
    const file = req.file;
    if (!file) {
      return res.status(400).json({ message: "File missing" });
