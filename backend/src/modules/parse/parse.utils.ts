...
const dedupeStr = (items: string[]): string[] =>
  [...new Map(items.map((s) => [s.toLowerCase().trim(), s.trim()])).values()].filter(
    Boolean,
  );
...
const parseName = (map: SectionMap): string => {
  ...
  for (const line of headerLines.slice(0, 10)) {
    ...
    if (
      /^(skills|experience|education|summary|profile|contact|objective|references|certifications|about)/i.test(
        line,
      )
    )
      continue;
    ...
    if (line.includes(",")) continue; // "Company, City" or "City, Country" patterns
...
const parseLocation = (map: SectionMap, rawContent: string): string => {
  ...
  const workModeMatch = rawContent.match(
    /\b(remote|work from home|wfh|hybrid|onsite|on-site|on site)\b/i,
  );
  ...
  for (const line of headerLines) {
    ...
    const cityMatch = line.match(
      /^([A-Z][A-Za-z\s]{1,30}),\s*([A-Z][A-Za-z\s]{1,30})$/,
    );
...
const parseSkills = (map: SectionMap, rawContent: string): string[] => {
  ...
  for (const line of rawContent.split("\n").map((l) => l.trim()).filter(Boolean)) {
    ...
    const avgWords =
      tokens.reduce((s, t) => s + t.split(/\s+/).length, 0) / tokens.length;
...
const parseExperienceYears = (
  map: SectionMap,
  rawContent: string,
): number | undefined => {
  // 1. Explicit "X years" statement
  const rangeMatch = rawContent.match(
    /(\d+(?:\.\d+)?)\s*(?:-|to)\s*(\d+(?:\.\d+)?)\s*\+?\s*(?:years|yrs|year)\b/i,
  );
  if (rangeMatch) return Number(parseFloat(rangeMatch[1]));

  const singleMatch = rawContent.match(
    /(\d+(?:\.\d+)?)\s*\+?\s*(?:years|yrs|year)\s+(?:of\s+)?(?:total\s+)?(?:work\s+|professional\s+|industry\s+|overall\s+)?experience/i,
  );
  ...
  const expBlock =
    getSection(map, "experience") ||
    getSection(map, "projects") ||
    rawContent;

  const currentYear = new Date().getFullYear();
  const currentMonth = new Date().getMonth() + 1;
  const ranges: Array<[number, number]> = []; // [startMonth, endMonth] as total months
  ...
  const yearOnlyRe =
    /(\d{4})\s*[-–—to]+\s*(\d{4}|present|current|now|till\s*date)/gi;
...
const parseWorkHistory = (
  ...
  const expBlock =
    getSection(map, "experience") || rawContent;

  // ── Pattern A: "Title at Company" / "Title @ Company" ────────────────────
  const atMatch = expBlock.match(
    /([A-Za-z][A-Za-z /\-]{3,60}?)\s+(?:at|@)\s+([A-Z][A-Za-z0-9 &.,'\-]{1,60}?)(?:\s*[\(,|\n]|\s*\d{4}|$)/,
  );
  if (atMatch) {
  ...
  const pipeMatch = expBlock.match(
    /([A-Za-z][A-Za-z0-9 &.,'\-]{1,60}?)\s*\|\s*([A-Za-z][A-Za-z0-9 &.,'\-]{1,60}?)\s*(?:\||\d{4})/,
  );
  if (pipeMatch) {
...
const parseSummary = (map: SectionMap, rawContent: string): string => {
  ...
  const summaryBlock = getSection(
    map,
    "summary",
    "contact", // some short resumes put bio in contact section
  );
  if (summaryBlock && summaryBlock.length > 40) {
    ...
    if (joined.length > 40) return joined.slice(0, 500);
  }
  ...
  for (const para of paragraphs) {
    ...
    if (avgLen > 30) return trimmed.slice(0, 500);
  }
...
const parseCurrentTitle = (
  ...
  const labelMatch = header.match(
    /(?:current title|job title|designation|position|role)\s*[:\-]\s*([A-Za-z][A-Za-z /\-]{3,80})/i,
  );
  ...
  const roleMatch = searchIn.match(
    /(?:i(?:'m| am) (?:a |an )?|working as (?:a |an )?|currently (?:a |an )?|serve(?:d|s)? as (?:a |an )?)([A-Za-z][A-Za-z /\-]{3,60}?)(?:\s+at\s|\s+with\s|\s+@\s|[,.\n]|$)/i,
  );
...
const parseCurrentCompany = (
  ...
  const header = map.get("__header__") ?? "";
  const labelMatch = header.match(
    /(?:current company|current employer|company|employer|organization)\s*[:\-]\s*([A-Z][A-Za-z0-9 &.,'\-]{1,80})/i,
  );
  if (labelMatch?.[1]) {
  ...
  const searchIn = header + "\n" + getSection(map, "summary");
  const atMatch = searchIn.match(
    /(?:\bat\b|\bwith\b)\s+([A-Z][A-Za-z0-9 &.,'\-]{1,60}?)(?:\s+(?:Inc|Ltd|LLC|Corp|Limited|Technologies|Tech|Solutions|Systems|Services|Global|Group)\.?)?(?=[,.\n]|$|\s+(?:as|where|since|from|and)\b)/,
  );
  if (atMatch?.[1]) {
  ...
  // 4. Corporate suffix
  const corpMatch = rawContent.match(
...
const parseJobTitle = (map: SectionMap, rawContent: string): string => {
  // Labeled field
  const labelMatch = rawContent.match(
    /(?:job title|position title|role|designation)\s*[:\-]\s*([A-Za-z0-9 &\-\/+]{3,100})/i,
  );
...
const parseBudget = (rawContent: string) => {
  const match = rawContent.match(
    /(?:budget|salary|compensation|ctc|package|remuneration)\s*[:\-]?\s*([^\n]{3,80})/i,
  );
  if (!match)
    ...
    (map.get("__header__") ?? "")
      ...
      .filter(
        (l) =>
          l &&
          !/[0-9@]/.test(l) &&
          !/^(Skills|Experience|Education|Summary|Profile|Contact|Company)/i.test(l),
      )[0] ||
    "";
