...
exports.sendCampaign = exports.createCampaign = exports.listCampaigns = exports.sendEmail = exports.listTemplates = void 0;
...
const transporter = env_1.default.SMTP_HOST
    ? nodemailer_1.default.createTransport({
        ...
        auth: env_1.default.SMTP_USER && env_1.default.SMTP_PASS
            ? { user: env_1.default.SMTP_USER, pass: env_1.default.SMTP_PASS }
            : undefined,
    })
...
const listCampaigns = async (req, res) => {
    const tenantId = req.user?.tenant_id;
    try {
        const result = await db_1.default.query("SELECT * FROM email_campaigns WHERE tenant_id = $1 ORDER BY created_at DESC", [tenantId]);
...
const createCampaign = async (req, res) => {
    ...
    try {
        const result = await db_1.default.query(`INSERT INTO email_campaigns (tenant_id, name, subject, body, status, recipient_count, created_by)
...
const sendCampaign = async (req, res) => {
    ...
    try {
        const campaignResult = await db_1.default.query("SELECT * FROM email_campaigns WHERE id = $1 AND tenant_id = $2", [id, tenantId]);
        ...
        const deliveredCount = sendResults.filter((promise) => promise.status === "fulfilled").length;
        const statusUpdate = deliveredCount === recipients.length ? "sent" : "failed";
        await db_1.default.query(`UPDATE email_campaigns SET status = $1, recipient_count = $2, delivered_count = $3, sent_at = now(), updated_at = now() WHERE id = $4`, [statusUpdate, recipients.length, deliveredCount, id]);
        res.json({
