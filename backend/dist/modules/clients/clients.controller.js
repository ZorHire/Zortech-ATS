...
const createClient = async (req, res) => {
    ...
    client_name, name, client_type, industry, company_size, website, linkedin, headquarters_location, operating_locations, 
    // Primary contact
    ...
    // AI
    preferred_skills, typical_roles, candidate_preference, hiring_strategy, interview_process, evaluation_criteria, 
    // Performance
    ...
    try {
        ...
        const clientResult = await dbClient.query(`INSERT INTO clients (
        ...
        if (Array.isArray(stakeholders) && stakeholders.length > 0) {
            for (const s of stakeholders) {
                if (!s.name?.trim())
                    continue;
                await dbClient.query(`INSERT INTO client_stakeholders (client_id, tenant_id, name, role, email, phone, timezone)
           VALUES ($1, $2, $3, $4, $5, $6, $7)`, [newClient.id, tenantId, s.name.trim(), s.role || null, s.email || null, s.phone || null, s.timezone || null]);
            }
        }
        await dbClient.query('COMMIT');
        // Return client with stakeholders
        const fullResult = await db_1.default.query(`SELECT c.*,
        COALESCE(json_agg(s.* ORDER BY s.created_at) FILTER (WHERE s.id IS NOT NULL), '[]') AS stakeholders
       FROM clients c
       LEFT JOIN client_stakeholders s ON s.client_id = c.id
       WHERE c.id = $1
       GROUP BY c.id`, [newClient.id]);
...
const updateClient = async (req, res) => {
    const { id } = req.params;
    const tenantId = req.user?.tenant_id;
    const { client_name, name, client_type, industry, company_size, website, linkedin, headquarters_location, operating_locations, primary_contact_name, contact_name, primary_contact_email, primary_contact_phone, alternate_contact, engagement_type, hiring_volume, active_requirements, client_priority, sla, working_hours, billing_model, currency, markup, payment_terms, invoice_cycle, billing_contact, contract_start, contract_end, msa_signed, nda_signed, preferred_skills, typical_roles, candidate_preference, hiring_strategy, interview_process, evaluation_criteria, positions_closed, avg_closure_time, interview_ratio, offer_acceptance_rate, preferred_channel, update_frequency, auto_report, account_manager, delivery_lead, recruiters, tags, notes, tier, logo_url, address, city, country, sla_hours, is_active, stakeholders, } = req.body;
    const resolvedName = client_name || name;
    const resolvedContactName = primary_contact_name || contact_name;
    const dbClient = await db_1.default.connect();
    try {
        await dbClient.query('BEGIN');
        const result = await dbClient.query(`UPDATE clients SET
        ...
        if (Array.isArray(stakeholders)) {
            ...
            for (const s of stakeholders) {
                if (!s.name?.trim())
                    continue;
                await dbClient.query(`INSERT INTO client_stakeholders (client_id, tenant_id, name, role, email, phone, timezone)
           VALUES ($1, $2, $3, $4, $5, $6, $7)`, [id, tenantId, s.name.trim(), s.role || null, s.email || null, s.phone || null, s.timezone || null]);
            }
        }
        await dbClient.query('COMMIT');
        const fullResult = await db_1.default.query(`SELECT c.*,
...
const deleteClient = async (req, res) => {
    ...
    const tenantId = req.user?.tenant_id;
    try {
        const result = await db_1.default.query('UPDATE clients SET deleted_at = now() WHERE id = $1 AND tenant_id = $2 RETURNING id', [id, tenantId]);
        if (result.rows.length === 0) {
            return res.status(404).json({ message: 'Client not found' });
        }
