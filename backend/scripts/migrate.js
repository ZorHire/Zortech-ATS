/**
 * Migration runner — no psql required.
 * Usage: node backend/scripts/migrate.js
 * Reads DATABASE_URL from root .env, runs all pending migrations in order.
 */

require("dotenv").config({ path: require("path").resolve(__dirname, "../../.env") });
const { Pool } = require("pg");

const DATABASE_URL = process.env.SERVER_DATABASE_URL;
if (!DATABASE_URL) {
  console.error("❌  SERVER_DATABASE_URL not found in .env");
  process.exit(1);
}

const pool = new Pool({ connectionString: DATABASE_URL });

// ─── Migration 001: Extend clients + client_stakeholders ─────────────────────
...
// ─── Migration 002: Per-user email config ────────────────────────────────────
...
const migrations = [
  { name: "001_extend_clients", sql: M001 },
  { name: "002_user_email_config", sql: M002 },
];
