...
export default function Header({ title, subtitle, actions }: HeaderProps) {
  ...
  return (
    <header className="px-10 py-8 flex items-center justify-between bg-white">
      <div>
        <h1 className="text-3xl font-black text-[#111111] tracking-tight">{title}</h1>
        {subtitle && <p className="text-sm font-bold text-gray-400 mt-1 uppercase tracking-widest">{subtitle}</p>}
      </div>
