...
import { Search, Filter, MapPin, Building2, Clock, Star, Mail, Phone, Plus, UserCheck, ChevronDown, CheckSquare, Square, Trash2, Send, X } from 'lucide-react';
...
export default function CandidatesPage() {
  ...
  const handleAddCandidate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const newCandidate = await api.post('/candidates', payload);
  ...
  const filtered = candidates.filter(c => {
    const matchSearch = [c.first_name, c.last_name, c.current_title || '', c.current_company || '', ...c.skills]
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <p className="text-sm text-gray-400 font-medium">
              Showing <span className="text-gray-900 font-bold">{filtered.length}</span> candidates
            </p>
          </div>
          <div className="flex gap-2">
            <button 
            ...
            <button className="text-xs font-bold px-4 py-2 bg-emerald-50 text-emerald-700 border border-emerald-100 rounded-xl hover:bg-emerald-100 transition-all">AI Match to Job</button>
          </div>
        </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-24 bg-white rounded-3xl border border-gray-100 shadow-sm">
            ...
            <p className="text-gray-900 font-bold text-lg">No candidates found</p>
            ...
            {filtered.map(candidate => (
              <div 
                ...
                className={`bg-white border rounded-2xl p-5 hover:shadow-xl hover:border-blue-200 transition-all cursor-pointer group relative overflow-hidden flex flex-col ${
                  selectedIds.has(candidate.id) ? 'border-blue-500 ring-2 ring-blue-500/10' : 'border-gray-100'
                }`}
              >
                <div 
                  onClick={(e) => toggleSelect(candidate.id, e)}
                  className={`absolute top-4 left-4 z-10 p-1 rounded-lg transition-all ${
                    selectedIds.has(candidate.id) ? 'text-blue-600 opacity-100' : 'text-gray-300 opacity-0 group-hover:opacity-100 hover:text-blue-500'
                  }`}
                >
                  {selectedIds.has(candidate.id) ? <CheckSquare size={20} /> : <Square size={20} />}
                </div>
                ...
                <div className="flex flex-col items-center text-center mt-2 mb-4">
                  ...
                  <p className="text-xs text-gray-500 font-medium mt-1 truncate w-full">{candidate.current_title}</p>
                </div>

                <div className="space-y-3 flex-1">
                  <div className="flex items-center justify-center gap-1.5 p-2 bg-gray-50 rounded-xl">
                    <Building2 size={13} className="text-gray-400" />
                    <span className="text-xs text-gray-600 font-bold truncate">{candidate.current_company}</span>
                  </div>

                  <div className="grid grid-cols-2 gap-2 text-[10px] font-bold text-gray-400 uppercase tracking-wider">
                    <div className="flex items-center gap-1.5"><MapPin size={12} className="text-blue-500" />{candidate.current_location}</div>
                    <div className="flex items-center gap-1.5"><Clock size={12} className="text-emerald-500" />{candidate.experience_years}y exp</div>
                  </div>

                  <div className="flex flex-wrap gap-1.5">
                    {candidate.skills.slice(0, 3).map(skill => (
                      <span key={skill} className="px-2 py-1 bg-blue-50/50 text-blue-700 text-[10px] font-bold rounded-lg border border-blue-100/50">
                        {skill}
                      </span>
                    ))}
                ...
                <div className="mt-5 pt-4 border-t border-gray-50 flex items-center justify-between">
                   <span className={`px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider ${sourceBadgeColors[candidate.source]}`}>
                    {sourceLabels[candidate.source]}
                  </span>
                  <div className="flex gap-1">
                    <button className="p-2 text-gray-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-all"><Mail size={14} /></button>
                    <button className="p-2 text-gray-400 hover:text-emerald-600 hover:bg-emerald-50 rounded-lg transition-all"><Phone size={14} /></button>
                  </div>
                </div>
              </div>
            ))}
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[60] p-4">
          <div className="bg-white rounded-[32px] shadow-2xl w-full max-w-2xl overflow-hidden animate-in fade-in zoom-in duration-200">
            <div className="px-8 py-6 border-b border-gray-100 flex items-center justify-between bg-gray-50/50">
              <h2 className="text-xl font-black text-[#111111] tracking-tight">Add New Candidate</h2>
